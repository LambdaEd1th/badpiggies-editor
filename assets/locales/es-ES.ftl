...
menu_night_vision_overlay = Superposición de visión nocturna
...
add_kind_prefab = Prefabricado
add_kind_parent = Principal
...
add_data_type_terrain_help = Terreno crea datos de terreno predeterminados y conserva el índice de prefabricado que seleccionó.
add_data_type_prefab_overrides_help = PrefabOverrides crea una carga de invalidación vacía que puede editar posteriormente en el panel de propiedades.
...
add_terrain = Adjuntar datos de terreno
...
level_warning_multiple_slingshots = Se encontraron { $count } objeto(s) «{ $name }». El juego original busca catapultas mediante consultas especiales de escena y puede comportarse de manera impredecible con más de una.
...
level_warning_missing_game_camera = No se encontró el objeto «{ $name }» aunque existe CameraSystem. El prefabricado CameraSystem enviado normalmente proporciona la GameCamera etiquetada como MainCamera que el código de gameplay resuelve mediante búsquedas Camera.main, MainCamera y GameCamera.
level_warning_missing_hud_camera = No se encontró el objeto «{ $name }» aunque existe CameraSystem. El código UI resuelve repetidamente la cámara HUD mediante FindGameObjectWithTag(«HUDCamera»).
...
error_pbkdf2_init = Error al inicializar PBKDF2 HMAC-SHA1
error_aes_encrypt_buffer_too_small = Búfer de cifrado AES demasiado pequeño
...
error_icon_layers_missing = Falta el activo integrado icon-layers.toml
error_icon_layers_not_utf8 = icon-layers.toml no es UTF-8 válido: { $name }
error_icon_layers_parse = Error al analizar icon-layers.toml: { $name }
error_bg_data_parse = Error al analizar bg-data.toml: { $name }
error_sprite_data_parse = Error al analizar sprite-data.toml: { $name }
error_level_refs_parse = Error al analizar level-refs.toml: { $name }
...
tool_preview_night_vision_powerup = Potenciador de visión nocturna
tool_preview_night_vision_active = Visión nocturna activa
...
# Level bounds editor
menu_edit_level_bounds = Editar límites del nivel…
win_level_bounds = Límites del nivel
bounds_pos_x = Posición X:
bounds_pos_y = Posición Y:
bounds_size_w = Ancho:
bounds_size_h = Altura:

# Save file viewer
menu_open_save = Abrir archivo de guardado…
save_viewer_title = Visor de guardado
...
save_edit_clear_all = Limpiar todas las entradas
save_edit_duplicate_all = Duplicar todas las entradas
