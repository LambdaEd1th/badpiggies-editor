...
menu_night_vision_overlay = Оверлей ночного видения
...
add_kind_prefab = Префаб
add_kind_parent = Родитель
...
add_data_type_terrain_help = Местность создаёт данные местности по умолчанию и сохраняет выбранный вами индекс префаба.
add_data_type_prefab_overrides_help = PrefabOverrides создаёт пустые данные переопределения, которые вы можете отредактировать позже в панели свойств.
...
add_terrain = Присоединить данные местности
...
level_warning_multiple_slingshots = Найдено { $count } объект(ов) «{ $name }». Оригинальная игра находит рогатки через специальные запросы сцены и может вести себя непредсказуемо с несколькими.
...
level_warning_missing_game_camera = Объект «{ $name }» не найден, даже если существует CameraSystem. Поставляемый префаб CameraSystem обычно предоставляет GameCamera с тегом MainCamera, которую код геймплея разрешает через поиски Camera.main, MainCamera и GameCamera.
level_warning_missing_hud_camera = Объект «{ $name }» не найден, даже если существует CameraSystem. Код UI многократно разрешает HUD камеру через FindGameObjectWithTag(«HUDCamera»).
...
error_pbkdf2_init = Не удалось инициализировать PBKDF2 HMAC-SHA1
error_aes_encrypt_buffer_too_small = Буфер шифрования AES слишком мал
...
error_icon_layers_missing = Встроенный ресурс icon-layers.toml отсутствует
error_icon_layers_not_utf8 = icon-layers.toml не является действительной кодировкой UTF-8: { $name }
error_icon_layers_parse = Не удалось разобрать icon-layers.toml: { $name }
error_bg_data_parse = Не удалось разобрать bg-data.toml: { $name }
error_sprite_data_parse = Не удалось разобрать sprite-data.toml: { $name }
error_level_refs_parse = Не удалось разобрать level-refs.toml: { $name }
...
tool_preview_night_vision_powerup = Усиление ночного видения
tool_preview_night_vision_active = Ночное видение активно
...
# Level bounds editor
menu_edit_level_bounds = Редактировать границы уровня…
win_level_bounds = Границы уровня
bounds_pos_x = Позиция X:
bounds_pos_y = Позиция Y:
bounds_size_w = Ширина:
bounds_size_h = Высота:

# Save file viewer
menu_open_save = Открыть файл сохранения…
save_viewer_title = Просмотр сохранения
...
save_edit_clear_all = Очистить все записи
save_edit_duplicate_all = Дублировать все записи
