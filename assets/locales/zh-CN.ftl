...
menu_night_vision_overlay = 夜视暗层
...
level_warning_multiple_slingshots = 检测到 { $count } 个 "{ $name }"。原游戏会通过特殊场景查询寻找弹弓，出现多个时可能产生不可预测行为。
...
level_warning_missing_game_camera = 在已有 CameraSystem 的情况下，没有检测到 "{ $name }"。原版 CameraSystem prefab 通常会提供带 MainCamera 标签的 GameCamera，而玩法代码会通过 Camera.main、MainCamera 和 GameCamera 查找来依赖它。
level_warning_missing_hud_camera = 在已有 CameraSystem 的情况下，没有检测到 "{ $name }"。UI 代码会反复通过 FindGameObjectWithTag("HUDCamera") 来直接获取 HUD 相机。
...
error_pbkdf2_init = 初始化 PBKDF2 HMAC-SHA1 失败
error_aes_encrypt_buffer_too_small = AES 加密缓冲区过小
...
error_icon_layers_missing = 嵌入资源缺少 icon-layers.toml
error_icon_layers_not_utf8 = icon-layers.toml 不是有效的 UTF-8: { $name }
error_icon_layers_parse = 解析 icon-layers.toml 失败: { $name }
error_bg_data_parse = 解析 bg-data.toml 失败: { $name }
error_sprite_data_parse = 解析 sprite-data.toml 失败: { $name }
error_level_refs_parse = 解析 level-refs.toml 失败: { $name }
