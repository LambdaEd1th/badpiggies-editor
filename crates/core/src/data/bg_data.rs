...
fn textureloader_materials() -> &'static HashMap<String, String> {
    static MATERIALS: OnceLock<HashMap<String, String>> = OnceLock::new();
    MATERIALS.get_or_init(parse::load_textureloader_materials)
}
...
pub fn atlas_for_material_guid(material_guid: &str) -> Option<&'static str> {
    ...
    let atlas_by_guid = ATLAS_BY_GUID.get_or_init(|| {
        textureloader_materials()
            .iter()
            .map(|(guid, asset_name)| (guid.clone(), parse::asset_filename(asset_name)))
            .collect()
    });
