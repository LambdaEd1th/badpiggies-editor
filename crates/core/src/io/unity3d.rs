...
pub fn list_text_assets_from_bytes(
    ...
    let scan_asset =
        |(asset_index, file): (usize, &unity_asset_binary::asset::SerializedFile)|
         -> AppResult<Vec<Unity3dTextAssetEntry>> {
    ...
    let indexed_assets = bundle.assets.iter().enumerate().collect();
    let groups: AppResult<Vec<Vec<Unity3dTextAssetEntry>>> =
        crate::parallel::try_map(indexed_assets, scan_asset);

    let mut seen = BTreeSet::new();
    let mut entries = groups?
        .into_iter()
        .flatten()
        .filter(|entry| seen.insert((entry.asset_path.clone(), entry.asset_index, entry.path_id)))
        .collect::<Vec<_>>();
