...
fn materialize_entries(
    ...
    let materialize = |entry: &UnityFsDirectoryEntry| -> AppResult<UnityFsEntry> {
    ...
    crate::parallel::try_map_ref(dir_entries, materialize)
}
...
fn decompress_data_blocks(
    ...
    let decompress = |&(kind, input, expected_size): &(u32, &[u8], usize)| {
        decompress_bytes(kind, input, expected_size)
    };
    let decompressed: AppResult<Vec<Vec<u8>>> = crate::parallel::try_map_ref(&jobs, decompress);
