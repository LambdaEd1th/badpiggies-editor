...
impl LevelRenderer {
    pub(super) fn handle_terrain_preset_mode(
        ...
        if ui.input(|i| i.key_pressed(crate::gpu2d::Key::Escape)) {
            self.terrain_preset_drag_start = None;
            self.terrain_preset_shape = None;
            self.panning = false;
            return;
    ...
    pub(super) fn handle_draw_terrain_mode(
        ...
        if matches!(
            ...
            if self.draw_terrain_active && ui.input(|i| i.key_pressed(crate::gpu2d::Key::Enter)) {
            ...
            if self.draw_terrain_active && ui.input(|i| i.key_pressed(crate::gpu2d::Key::Escape)) {
                self.draw_terrain_active = false;
                self.draw_terrain_points.clear();
                self.clear_terrain_draw_continuation_anchor();
            }
