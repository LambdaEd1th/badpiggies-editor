...
pub struct TerrainDrawData {
    ...
    pub fill_mesh: Option<crate::gpu2d::Mesh>,
...
/// Build the fill mesh as an crate::gpu2d::Mesh with the fill color applied to all vertices.
/// UV coordinates are tiled based on world position / TILE_SIZE.
fn build_fill_mesh(td: &TerrainData, offset: Vec3) -> Option<crate::gpu2d::Mesh> {
    ...
    let color = crate::gpu2d::Color32::from_rgba_unmultiplied(r, g, b, a);
    ...
    if !mesh.is_valid() {
        log::error!(
            "build_fill_mesh: 2D mesh INVALID! verts={} indices={}",
            mesh.vertices.len(),
            mesh.indices.len()
        );
    }
...
impl LevelRenderer {
    pub(in crate::renderer) fn draw_terrain_index(
        ...
        _canvas_center: crate::gpu2d::Vec2,
