// Port of Unity shader "_Custom/Text Shader With Z Test".
...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    out.color = clamp(u.color, vec4<f32>(0.0), vec4<f32>(1.0));
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let alpha = textureSample(main_tex, main_sampler, in.uv).w * in.color.w;
