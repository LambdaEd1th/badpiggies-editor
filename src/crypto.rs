...
fn aes_decrypt(key: &[u8; 32], iv: &[u8; 16], ciphertext: &[u8]) -> Result<Vec<u8>, String> {
...
fn verify_sha1(data: &[u8]) -> Result<&[u8], String> {
    if data.len() < 20 {
        return Err("File too short for SHA1 hash".into());
    }
    let (hash_bytes, payload) = data.split_at(20);
    let computed = Sha1::digest(payload);
    if computed.as_slice() != hash_bytes {
        return Err("SHA1 hash mismatch — file may be corrupted".into());
    }
    Ok(payload)
}
...
pub fn decrypt_save_file(file_type: &SaveFileType, data: &[u8]) -> Result<Vec<u8>, String> {
    let (key, iv) = derive_key_iv(file_type.password());
